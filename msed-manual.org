...
implementation it most likely will write outside the intended memory
space and corrupt memory.
...
Note!  If you have song repeat enabled, the player routine will become
stuck in an endless loop if a starting track on any channel contains
no notes or only zero-length notes.  This will freeze the whole
computer!
...
The control commands are internally known as SCODEs.
...
| ~v~        | View output wave (useful to check BShift modulation)         |
| ~w~        | Go to wave editor                                            |
...
the other nybble.  However, with careful crafting of slope values it
may be possible to make fine changes to both nybbles.
...
| 0x1860 |                   | ~mem_MeterBuffer~           |        | "Imploder meter buffer"                                                 |
