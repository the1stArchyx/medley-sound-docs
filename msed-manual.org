...
| ~t~              | Trace mode (useful only while playing a track, but often broken!)      |
| Amiga + ~b~      | Set block                                                              |
| Amiga + ~c~      | Copy block                                                             |
...
Some operations affect the octave number of waves in a way that
potentially makes the waves play incorrectly.  This happens when the
octave number goes beyond its functional range of 0..7.  This is
something to look further into, both in the wave editor and the replay
routine, to understand how the octave number affects the playback of
samples.
